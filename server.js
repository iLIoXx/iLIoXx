...
${videos.map(video2 => `[**${++index}**] **${video2.title}**`).join("\n")}`
    ...
    if (!args[1])
      return msg.channel.send(
        `:musical_keyboard:  مستوى الصوت **${serverQueue.volume}**`
      );
    serverQueue.volume = args[1];
    serverQueue.connection.dispatcher.setVolumeLogarithmic(args[1] / 50);
    return msg.channel.send(
      `:musical_keyboard:  تم تغير الصوت الي **${args[1]}**`
    );
  } else if (command === `np`) {
    if (!serverQueue) return msg.channel.send("لا يوجد شيء حالي ف العمل.");
    const embedNP = new Discord.RichEmbed().setDescription(
      `<a:mu:636594533051793408> الان يتم تشغيل : **${serverQueue.songs[0].title}**`
    );
      ...
      serverQueue.textChannel
        .send({
          embed: new Discord.RichEmbed()
            .setURL(fuck.url)
            .addField(":musical_note:  Time The Video .. :", `${song.time}`,true)
            .addField(":pencil: The Singular .. :", `${song.best}`, true)
            .setColor("#ff0000")
        })
        .then(love => {
            ...
            love.react("sw").then(r => {
              love.react("wa").then(r => {
                let likee = (reaction, user) =>
                  reaction.emoji.name === "ds" && user.id === msg.author.id;
                let dislikee = (reaction, user) =>
                  reaction.emoji.name === "sd" && user.id === msg.author.id;
                let cnn = (reaction, user) =>
                  reaction.emoji.name === "sw" && user.id === msg.author.id;
...
client.login("NjM5Mzg5MDY1NzUzNTI2Mjgy.Xbqj3g.GHGorA54mir72VkZClPC7o6_v_I");
